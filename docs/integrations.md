...
`CLOUDBEDS_PROPERTY_ID` is only actually required if your Cloudbeds account has
more than one property - `make doctor` checks it lazily, the first time that
turns out to matter, rather than failing on it up front. Leave it blank on a
