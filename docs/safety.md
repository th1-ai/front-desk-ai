...
You can still `approve`, `edit` or `reject` items while in shadow - those
decisions are recorded (and are what teaches the coach, see
`workflows/85-coach-weekly.md`), but `send` refuses every item regardless,
even one you just approved. Nothing changes that until `mode: live`. Because
of that, `workflows/90-go-live.md` has you run `python3 tools/review.py stale`
once, right before the flip, so a backlog approved during shadow (which may
be days or weeks old by then) does not all go out at once the moment live
mode starts.
