...
`test_frontdesk_*.py` must never read this repo's own `config/hotel.yaml` /
...
test, so this fixture only isolates the frontdesk-specific test files -
...
def isolated_config(request, tmp_path, monkeypatch):
    ...
    if not module_file.startswith("test_frontdesk_"):
        yield
        return
