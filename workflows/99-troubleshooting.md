...
- **`specialist booking`: enabled but fixtures/hotel/experiences.json is
  missing.** Add the file (`workflows/20-specialist-booking.md`) or turn
  `subagents.specialist_booking.enabled` back to `false`.
...
- `tools/demo.py` forces `llm.provider=mock` and reads
  `fixtures/inbound/*.json`, `fixtures/inbound/messages.json` and
  `fixtures/hotel/reservations.json` - if you deleted or renamed those
...
Check `python3 tools/review.py show <id>` for the item's `error` field first
- `tools/review.py send` records exactly why `finalize_booking` failed (a
guardrail refusal, an unmatched room type, a closed adapter) rather than
silently dropping the booking. `data/exports/pms_writes.csv` shows what a
`csv`-mode PMS would have received instead of a live write.

## A confirmation or reminder never shows up in the queue

- Run `python3 tools/run.py --once --confirmations` directly and read its
  summary line - a booking is only ever queued once (idempotent dedup on
  `kind:ref`), so a booking you have already seen produces `0 booking(s)
  seen` on purpose.
- Check the reminder was not already lapsed (`tools/confirmations.py`
  drops a slot whose due time has already passed).

## The triage or draft step gets a message wrong

Fix it in the review queue first (`edit`, not `reject`, so the correction is
recorded as a learning), then let `workflows/85-coach-weekly.md` turn a
repeated pattern into a proposal. Prompts are plain markdown -
`prompts/triage.md` and `prompts/draft.md` can be edited directly too.
